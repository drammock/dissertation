...
\subsection{Listener uncertainty\label{sec:uncertainty}}
...
Early research into listener uncertainty showed higher word-recognition scores for blockwise presentation of a single talker compared to mixed-talker blocks \citep{SommersEtAl1994}, suggesting that listeners can accomodate (or “tune in”) to a particular voice when talker identity is predictable (cf. \citealt{VanEngen2012}).  In contrast, trial-to-trial variability in overall amplitude of stimuli does not seem to affect speech perception \citep{SommersEtAl1994}.  % TODO: need better transition between this and next paragraph

In general, experiments exploring stimulus uncertainty using speech maskers are less common, presumably due to the difficulty of controlling and quantifying the spectrotemporal similarity of speech maskers (as can be done fairly readily with tone complexes).  Nonetheless, a few key studies using speech targets and/or speech maskers bear mentioning here.  First, it has been shown that holding the semantic content of the masker stream fixed leads to a release from masking, suggesting that masking due to listener uncertainty can arise from (semantic) variation in the masker \citep{BrungartSimpson2004}.  A second study showed that uncertainty about the spatial location of the target speech (in the presence of two competing, spatially distinct speech streams) resulted in dramatically lower target perception success than cases where the location of the target speech was known \citep{KiddEtAl2005a}.  A third study examined the effect of listener uncertainty due to variations in SNR, talker identity in the masking stream, and constancy of masking stimulus tokens.  Surprisingly, none of these factors resulted in a significant release from masking \citep{FreymanEtAl2007}.  % TODO: also look at Ericson Et al 2004, and maybe Arbogast & Kidd 2002
...
\subsubsection{Duration, speech rate, and rhythm}
speech rate \citep{Tolhurst1957a} (normal, prolonged, staccato).  Evidence is mixed as to whether speaking rate is predictive of intelligibility \citep{SommersEtAl1994, BradlowEtAl1996, KrauseBraida2002}.
...
Rhythmic distance affects bilingual infants: \citep{SundaraScutellaro2011}.  
Intensity patterns: word-by-word SNR, glimpsing, audibility, word stress and phrasal stress.  
...
intensities \citep{BondMoore1994}.  Glimpsing & word-by-word SNR, relation to "trail-off".  
