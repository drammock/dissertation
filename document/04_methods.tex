...
In order to maximize inter-speaker agreement in number of durational units for a given sentence, the intensity-based syllablewise method of segmentation was chosen over the more common spectrogram-based phonemic\slsh{}allophonic segmentation method \citep[\eg][]{PetersonLehiste1960, TurkEtAl2006}.  For the same reason, the method of marking only periodic/aperiodic transitions \citep[\eg][]{xxx} was also avoided.  Whereas there were often inter-speaker disagreements on the number of segments (\eg, eight in [pʰɑɹk̚.tʰɹʌk] \vs\ nine in [pʰɑɹkt.tʰɹʌk] for “parked truck”) or the number of voiced\slsh{}voiceless transitions (\eg, [ə.pʰ.ɑɾə.tʰ.i] \vs\ [ə.pʰ.ɑ.t.əv.tʰ.i] for “a pot of tea”), virtually all sentences in the corpus showed inter-talker agreement on number of syllables.  The rare disagreements were cases of extreme reduction, \eg, sentence 22–07 “It is hard to erase blue or red ink”, in which one talker contracted the initial syllables to “It’s”.  In extreme cases such as this, the sentence was excluded from resynthesis, but in most such cases it was still possible to separate the reduced form into two syllable-like units using the same acoustic landmarks mentioned above.  Such sentences were therefore retained to avoid introducing a systematic bias by eliminating the most heavily reduced sentences.
...
\begin{figure}
	\begin{centering}
	\includegraphics{figures/intensity/intensity.eps}
	\caption[Intensity scaling in resynthesis]{Illustration of the method used to scale intensity (still under construction).  (a)~Waveform (black) and intensity contour (dashed blue line) of a prosodic donor.  (b)~Waveform (black), intensity contour (dashed blue line), and inverted intensity contour (dotted red line) of the target sound.  (c)~Waveform (black) and intensity contour (dashed blue line) of target sound after multiplying by the inverse waveform.  (d)~Waveform (black) and intensity contour of target sound after multiplying by time-warped intensity contour of prosodic donor.\label{fig:IntenManip}}
	\end{centering}
\end{figure}
...
\begin{table}
	\caption[Listener demographics]{Listener demographics for Experiments~1 and~2.\label{tab:ListDemo}}
	\centering
