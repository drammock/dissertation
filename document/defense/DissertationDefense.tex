...
\begin{frame}{Definition of prosody}
	...
	\item<2-> \fo, duration \& intensity modulated at smaller time scales too:
		...
		\item Segmental level
			...
			\end{itm}
		\end{itm}
	\item<3> Phrase-level modulations in \fo{} \& duration > modulations at smaller time scales
%		\begin{itm}
...
\begin{frame}{Segregating prosodic \& non-prosodic contributions (1 of 3)}
	\begin{itm}
...
%	Summary of statistical model\\ \vspace{\baselineskip}
	\begin{itm}
%	\item[] Summary of statistical model
	\item[] 
		{\small
		\begin{tabu} spread 3em {X[1,m,l] X[1,m,l] X[-1,m,c]}
			...
			\midrule
				           & Talker \ac{a} & baseline \\
			Prosodic donor & Talker \ac{b} & +0.3 \\
				           & Talker \ac{c} & −0.6 \\
			\taburulecolor{lgray}\midrule
				           & Talker \ac{a} & baseline \\
			Signal donor   & Talker \ac{b} & −1.7 \\
				           & Talker \ac{c} & −1.3 \\
			\midrule
...
\begin{frame}{\Ph{} acoustic analyses (overview)}
	...
	\item[] %{\small
		\begin{tabu} spread 1em {X[m,l] X[-1,m,c]}
			...
			\midrule
			Overall intelligibility & {\color{rdd}\ac{a}} > {\color{grn}\ac{b}} > {\color{blu}\ac{c}} \\
			\taburulecolor{lgray}\midrule
			Non-prosodic factors    & {\color{rdd}\ac{a}} > {\color{blu}\ac{c}} > {\color{grn}\ac{b}} \\
			\midrule
			Prosody                 & {\color{grn}\ac{b}} > {\color{rdd}\ac{a}} > {\color{blu}\ac{c}} \\
			\taburulecolor{black}\bottomrule
		\end{tabu}
%	} %small
	\item[]
...
\begin{frame}{\Ph{} acoustic analyses (overview)}
	...
	\item[] Which acoustic predictors match those patterns?
		\begin{itm}
		\item[]
		\item Overall intelligibility ({\color{rdd}\ac{a}} > {\color{grn}\ac{b}} > {\color{blu}\ac{c}})
			\begin{itm}
			\item Unreduced stop consonants
			\item Vowel space size: area of polygon based on means
			\item[]
			\end{itm}
		\item<2-> Non-prosodic factors ({\color{rdd}\ac{a}} > {\color{blu}\ac{c}} > {\color{grn}\ac{b}})
			\begin{itm}
			...
			\item[]
			\end{itm}
		\item<3> Prosody ({\color{grn}\ac{b}} > {\color{rdd}\ac{a}} > {\color{blu}\ac{c}})
			\begin{itm}
...
\begin{frame}{\Ph{} acoustic analyses (prosody)}
	...
	\item[] \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{../figures/posthocs/ProsodicMeasures.eps}
	...
	\begin{columns}[t]
		...
		\begin{column}{0.5\textwidth}
			...
			\item Talker \ac{c}’s creaky voicing inflates \fo~range, dynamicity, \& velocity
				...
				\item[]
				\end{itm}
			\item<2> If we ignore {\color{blu}\ac{c}}, prosody predicts {\color{grn}\ac{b}}~>~{\color{rdd}\ac{a}}.  Matches:
				\begin{itm}
				\item Mean \fo{} range
...
\begin{frame}{Things I could say a lot more about...}
	...
	\item<1,2> Why does \# of release bursts follow  overall intelligibility pattern instead of non-prosodic pattern?
		\begin{itm}
		\only<2>{%
		\item Prosody affects prominence: phrasal stress, domain-initial strengthening \etc. Speech rate may also play a role.
		}
		\item[]
		\end{itm}
	\item<1,3> Why does area of vowel means polygon follow overall intelligibility pattern, but area of convex hull follows non-prosodic pattern?
		\begin{itm}
		\only<3>{%
		\item Vowel formant values came from stressed vowels in keywords throughout sentence.  Thus differences in extreme \vs\ reduced forms (and means measured from them) are likely to reflect phrasal accents as well as segment-level talker habits.  Convex hull tracks \emph{\hl extreme} values only, thus “ignores” prosodically-driven reduction.
		}
		\item[]
		\end{itm}
	\item<1,4> About those keywords... 
		\begin{itm}
		\only<4>{%
		...
		\item[]
		\end{itm}
	\item<1,5> What about familiarity?
		\begin{itm}
		\only<5>{%
