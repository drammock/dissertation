...
\begin{frame}{What determines intelligibility? (1 of 4)}{}
...
\begin{frame}{What determines intelligibility? (2 of 4)}{}
{\str acoustic properties of speech (cont.)}
	\begin{itm}
	\item Duration \& speech rate: equivocal results
		\begin{itm}
		\item \emp{Long vowels, systematic pauses} associated with intelligibility\fnr{BondMoore1994}\refcomma\fnr{PichenyEtAl1986}%\savefnOne
...
%		\item Clear speech with pauses removed doesn’t get less intelligible\footnote[frame]{Choi1987 - MIT bachelor’s project}
	...
	\item<2> Intensity
		\begin{itm}
		\item Majority of studies use \ac{rms} normalized stimuli \& still see intelligibility variation
		\item One study without normalization: no significant intensity variation despite intelligibility variation\footnotemark[6] % BondMoore1994
		\item Where it might matter:\\stationary masker + phrasal intensity patterns = variable word-by-word \ac{snr}
		\end{itm}
%	\item[]
	\end{itm}
\end{frame}

\begin{frame}{What determines intelligibility? (3 of 4)}{}
{\str acoustic properties of speech (cont.)}
	\begin{itm}
	\item Vowel space size: mixed results, inconsistent measurement methods
		\begin{itm}
		\item F2 and/or F1 range\fnr{BondMoore1994}\refcomma\fnr{BradlowEtAl1996}\refcomma\fnr{HazanMarkham2004}
		\item Area of polygon formed by vowel means\footnotemark[11]\refcomma\fnr{Neel2008}\refcomma\footnotemark[14]%\fnr{McCloyEtAl2012} % BradlowEtAl1996
		\item Mean distance of vowels from center of vowel space\footnotemark[11]
		\item Area of convex polygonal hull encompassing all vowel tokens\footnotemark[15]%\fnr{McCloyEtAl2013}
		\item[]
		% TODO: add illustrations of these
		\end{itm}
	\item<2-> Consonantal reduction
		\begin{itm}
		\item Intelligible speech associated with more stop release bursts\footnotemark[16]%\fnr{PichenyEtAl1986}
		\item Consonants (quiet) more easily masked than vowels (loud), but high frequency cues may escape speech-shaped noise\footnotemark[17]%\fnr{PhatakAllen2007}
%		\item Vowels contain ⅔ of information in speech signal\fnr{KewleyPortEtAl2007}\refcomma\fnr{FogertyKewleyPort2009} %⅓
		\item[]
		\end{itm}
	\item<3> Key problem: acoustic dimensions covary in natural speech
%		\begin{itm}
%		\item Clear speech, Lombard, child-directed, sociolinguistic register, \etc.
%		\end{itm}
	\end{itm}
	{\tiny \vspace{0.3\baselineskip}}
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			
		\end{column}
		\begin{column}{0.5\textwidth}
			{\tiny \footnotemark[14]\citet*{McCloyEtAl2012}.\\[0.3\baselineskip]\footnotemark[15]\citet*{McCloyEtAl2013}.\\[0.3\baselineskip]\footnotemark[16]\citet*{PichenyEtAl1986}.\\[0.3\baselineskip]\footnotemark[17]\citet*{PhatakAllen2007}.\\{}}
		\end{column}
	\end{columns}
	{\footnotesize\vspace{-5\baselineskip}}
\end{frame}

\begin{frame}{What determines intelligibility? (4 of 4)}{}
{\str talker \& listener properties}
	...
	\item Listener familiarity with language/dialect
		...
		\item[]
		\end{itm}
	\item<2-> Intrinsic gender differences?\fnr{BradlowEtAl1996}\refcomma\fnr{HazanMarkham2004}
		\begin{itm}
		\item[]
		\end{itm}
...
\begin{frame}{Why use resynthesis?}
	...
	\item Top-down approach: how much of intelligibility is prosodic?
		\begin{itm}
		\item Swapped prosody of talkers that varied in intelligibility
			\begin{itm}
			\item \fo, duration, intensity
...
\begin{frame}{Resynthesis (3 of 3)}
	\begin{itm}
		...
		\item \fo, duration \& intensity swapped between all possible pairs of talkers
			...
			\item Known from previous intelligibility experiments: \ac{a} > \ac{b} > \ac{c}
			\item[]
			\end{itm}
		\item (Show demo)
	\end{itm}
...
\begin{frame}{Experimental design}
	\begin{itm}
	\item 90 semantically unrelated sentences
		\begin{itm}
		\item 810 stimuli total (270 unmodified, 540 resynthesized)
		\item[]
		\item Each listener heard 90 stimuli (each sentence once, random order)
		\item[]
...
%			\end{itm}
	...
	\begin{itm}
	\item[]
	\item<2>[] Different talkers rely on different aspects of speech to achieve intelligibility
	\end{itm}
\end{frame}
...
%	} %small
	...
	\item<2->[] Which acoustic predictors match those patterns?
		\begin{itm}
		\item<3> Can we segregate predictors tracking \emp{prosodic} contributions to intelligibility \vs\ those tracking \emp{non-prosodic} contributions? 
		\end{itm}
	\end{itm}
\end{frame}
...
\begin{frame}{\Ph{} acoustic analyses (overview)}
	...
	\item[] Which acoustic predictors match those patterns?
		...
		\item Overall intelligibility (\redd{a} > \grnn{b} > \bluu{c})
			...
			\item Mean syllable duration (sort of)
			\item[]
			\end{itm}
		\item<2-> Non-prosodic factors (\redd{a} > \bluu{c} > \grnn{b})
			...
			\item Variability in syllable duration
			\item[]
			\end{itm}
		\item<3> Prosody (\grnn{b} > \redd{a} > \bluu{c})
			\begin{itm}
			\item Nothing matches! (a few things come close...) % (intensity dynamicity comes close)
			\end{itm}
		\end{itm}
	\end{itm}
\end{frame}

\begin{frame}{\Ph{} acoustic analyses (\fo)}
	\begin{itm}
	\item[] Relative magnitudes of \fo{} metrics
	\item[] \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{../figures/posthocs/ProsodicMeasuresPitchOnly.eps}
...
\begin{frame}{\Ph{} acoustic analyses (\fo)}
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			\begin{itm}
			\item[] \includegraphics[width=\columnwidth,height=0.75\textheight,keepaspectratio]{../figures/posthocs/PitchTracks.pdf}
...
\begin{frame}{\Ph{} acoustic analyses (intensity)}
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			\begin{itm}
			\item[] Relative magnitudes of intensity metrics
			\item[] \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{../figures/posthocs/ProsodicMeasuresIntensityOnly.eps}
			\end{itm}
		\end{column}
		\begin{column}{0.4\textwidth}
			\begin{itm}
			\item<2> Mean dynamicity tracks with prosody (\grnn{b}~>~\redd{a}~>~\bluu{c}), but only \grnn{b} \vs\ \bluu{c} is significant
			\end{itm}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{\Ph{} acoustic analyses (intensity)}
	...
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			\begin{itm}
			\item[] \includegraphics[width=\columnwidth,height=0.75\textheight,keepaspectratio]{../figures/posthocs/IntensityTracks.pdf}
			\end{itm}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{itm}
...
\begin{frame}{Discussion: Unreduced stop consonants}
	\begin{itm}
	\item[] Why does \# of unreduced stop consonants follow \emp{overall intelligibility} pattern rather than \emp{non-prosodic}?
		\begin{itm}
		\item[]
		\item<2-> Prosody affects prominence
		\item[]
		\item<3-> Prominence (partially) determines stop articulation
		\item[]
		\item<4> Speech rate, phrasal stress, domain-initial strengthening may all play a role
		\end{itm}
	\end{itm}
\end{frame}

\begin{frame}{Discussion: Vowel space metrics}
	\begin{itm}
...
\begin{frame}{Acknowledgments}
	\begin{itm}
		...
		\item[] 
		\item[] Jennifer~Haywood, Valerie Freeman, Gus~McGrath, Steve~Moran, Darren~Tanner, members~of~the~UW~Phonetics~Lab
		\item[] 
...
\begin{frame}{Things I could say a lot more about...}
	...
	\item<1,2> About those keywords... 
		...
		\item[]
		\end{itm}
	\item<1,3> What about familiarity?
		\begin{itm}
		\only<3>{%
...
\begin{frame}{\Ph{} acoustic analyses (duration)}
	\begin{itm}
	\item[] Relative magnitudes of duration metrics
	\item[] \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{../figures/posthocs/SyllableDuration.eps}
	\end{itm}
\end{frame}
